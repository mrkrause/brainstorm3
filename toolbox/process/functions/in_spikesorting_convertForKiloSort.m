...
%   [F, TimeVector] = in_fread(sFile, ChannelMat, iEpoch, SamplesBounds, iChannels, ImportOptions)
    ...
    % KILOSORT PROBABLY USES INT16 AS INPUT CONFIRM !!!!!!
    % THIS IS WHAT THEY HAD IN THE make_eMouseData
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    F = F*10^6;  % This assumes that F signals are in V. You need to convert it to uV so you have big numbers and int16 precision doesn't zero it out.            
    fwrite(fid, F,'int16');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
